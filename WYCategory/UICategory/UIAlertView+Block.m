...
+(UIAlertView *)showAlertViewWithTitle:(NSString *)title withMessage:(NSString *)msg withAction:(WYUIAlertViewBlock)block{
    ...
    if(block){
        ...
        a = [[UIAlertView alloc] initWithTitle:mTitle message:mMessage delegate:self cancelButtonTitle:@"确认" otherButtonTitles:@"取消", nil];
        
        [a addActionHandler:^(NSInteger index) {
            block(index);
        }];
    }else {
        a = [[UIAlertView alloc] initWithTitle:mTitle message:mMessage delegate:self cancelButtonTitle:@"确认" otherButtonTitles: nil];
    }
    [a show];
    return  a;
}
